...
import UIKit
import AVFoundation
@objc public class MediaComposition: NSObject {
    /// 视频合成后地址 默认 tmp/imagesComposition.mp4
    @objc public var outputPath: String?
    /// 视频分辨率
    @objc public var naturalSize: CGSize = CGSize(width: 720, height: 1280)
    /// 每张图片展示的时间
    @objc public var picTime: Float = 3
    /// 帧率
    @objc public var frameNumber: Int = 25
    /// 视频背景 本地地址 默认black.mp4
    @objc public var videoResource: String?
    /// 视频原音静音
    @objc public var isMute: Bool = true
...
extension MediaComposition {
    ...
    ///   - success: 成功回调 合成视频地址
    ///   - failure: 失败回调
    @objc public func imagesVideoAnimation(with images:[UIImage], progress: ProgressBlock?, success: SuccessBlock?, failure: FailureBlock?){
        ...
        let tempDuration = Float(images.count) * picTime + Float(0.0005 * Float(images.count))
...
extension MediaComposition {
    ...
    private func positionAni(layers: [CALayer]){
        for (index, layer) in layers.enumerated() {
            ...
            animation.beginTime = 0.0005 + Double(picTime * Float(index))
            animation.fromValue = NSValue.init(cgPoint: CGPoint.init(x: naturalSize.width * CGFloat(index), y: 0))
//            animation.fromValue = NSValue.init(cgPoint: CGPoint.init(x: 0, y: 0))
